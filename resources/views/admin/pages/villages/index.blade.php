...
@include('admin.pages.villages.modal')
...
<script>
    $(document).ready(function() {
        ...
        let table =  $('#villages-table').DataTable( {
            responsive : true,
            processing : true,
            serverSide : true,
        ...
        $('#villages-table').on('click', '.btn-delete', function(){
            let id = $(this).data('id');
            ...
            swalWithBootstrapButtons.fire({
                ...
                if ( result.value ) {
                    $.ajax({
                        ...
                        url: "{{ url(AIIASetting::getValue('admin_base_route').'/villages') }}/"+id,
                        type: "delete",
                        dataType: "json",
        ...
        $('#villages-table').on('click', '.btn-update', function(){
            $('#frm-insert').find('.text-error').remove();
            let id = $(this).data('id');
            $('#frm-insert').attr('action', "{{ url(AIIASetting::getValue('admin_base_route').'/villages') }}/"+id).attr('method', 'PUT');
            $('#title-modal').text('Update VIllages');
            let village = $(this).data('village');
            $('#name').val(village.name);
            $('#subdistrict').append('<option value="'+village.subdistrict.id+'" selected="selected">'+village.subdistrict.name+'</option>');
        ...
        $('#btn-add').on('click', function() {
            ...
            $('#frm-insert').attr('action', "{{ route('admin.villages.store') }}").attr('method', 'POST');
            $('#title-modal').text('Add Village');
            $('#frm-insert')[0].reset();
            $('#mdl-insert-update').modal('show');
        });

        $('.subdistrict-ajax').select2({
