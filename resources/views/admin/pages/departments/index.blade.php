...
@include('admin.pages.departments.modal')
...
<script>
    $(document).ready(function() {
                ...
                { data : 'created_at', name : 'created_at' },
                {
                    ...
                    render: function(data){
                        return `<div class="text-center">
                                    <button type="button" data-id="${data.id}" data-department='${JSON.stringify(data)}' class="btn btn-outline-primary btn-fw btn-update btn-sm">
            ...
            swalWithBootstrapButtons.fire({
                ...
                if ( result.value ) {
                    $.ajax({
                        ...
                        url: "{{ url(AIIASetting::getValue('admin_base_route').'/departments') }}/"+id,
                        type: "delete",
                        dataType: "json",
        ...
        $('#departments-table').on('click', '.btn-update', function(){
            $('#frm-insert').find('.text-error').remove();
            let id = $(this).data('id');
            $('#frm-insert').attr('action', "{{ url(AIIASetting::getValue('admin_base_route').'/departments') }}/"+id).attr('method', 'PUT');
            $('#title-modal').text('Update Department');
            let department = $(this).data('department');
            $('#code').val(department.code);
            $('#name').val(department.name);
            $('#pic').val(department.pic);
            $('#pic_email').val(department.pic_email);
        ...
        $('#btn-add').on('click', function() {
            ...
            $('#frm-insert').attr('action', "{{ route('admin.departments.store') }}").attr('method', 'POST');
