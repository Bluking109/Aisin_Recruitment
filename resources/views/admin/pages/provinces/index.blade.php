...
@include('admin.pages.provinces.modal')
...
<script>
    $(document).ready(function() {
        ...
        let table =  $('#provinces-table').DataTable( {
            responsive : true,
            processing : true,
            serverSide : true,
        ...
        $('#provinces-table').on('click', '.btn-delete', function(){
            let id = $(this).data('id');
            ...
            swalWithBootstrapButtons.fire({
                ...
                if ( result.value ) {
                    $.ajax({
                        ...
                        url: "{{ url(AIIASetting::getValue('admin_base_route').'/provinces') }}/"+id,
                        type: "delete",
                        dataType: "json",
        ...
        $('#provinces-table').on('click', '.btn-update', function(){
            $('#frm-insert').find('.text-error').remove();
            $('#password-wrapper').hide();
            let id = $(this).data('id');
            $('#frm-insert').attr('action', "{{ url(AIIASetting::getValue('admin_base_route').'/provinces') }}/"+id).attr('method', 'PUT');
            $('#title-modal').text('Update Province');
            let province = $(this).data('province');
            $('#name').val(province.name);
            $('#email').val(province.email);
        ...
        $('#btn-add').on('click', function() {
            ...
            $('#frm-insert').find('.text-error').remove();
            $('#frm-insert').attr('action', "{{ route('admin.provinces.store') }}").attr('method', 'POST');
            $('#title-modal').text('Add Province');
            $('#frm-insert')[0].reset();
            $('#mdl-insert-update').modal('show');
        });
