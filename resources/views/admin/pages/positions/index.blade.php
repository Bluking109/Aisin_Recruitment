...
@include('admin.pages.positions.modal')
...
<script>
    $(document).ready(function() {
                ...
                { data : 'created_at', name : 'created_at' },
                {
                    ...
                    render: function(data){
                        return `<div class="text-center">
                                    <button type="button" data-id="${data.id}" data-position='${JSON.stringify(data)}' class="btn btn-outline-primary btn-fw btn-update btn-sm">
            ...
            swalWithBootstrapButtons.fire({
                ...
                if ( result.value ) {
                    $.ajax({
                        ...
                        url: "{{ url(AIIASetting::getValue('admin_base_route').'/positions') }}/"+id,
                        type: "delete",
                        dataType: "json",
        ...
        $('#positions-table').on('click', '.btn-update', function(){
            $('#frm-insert').find('.text-error').remove();
            let id = $(this).data('id');
            $('#frm-insert').attr('action', "{{ url(AIIASetting::getValue('admin_base_route').'/positions') }}/"+id).attr('method', 'PUT');
            $('#title-modal').text('Update Position');
        ...
        $('#btn-add').on('click', function() {
            ...
            $('#frm-insert').attr('action', "{{ route('admin.positions.store') }}").attr('method', 'POST');
            $('#title-modal').text('Add Position');
            $('#frm-insert')[0].reset();
            $('#mdl-insert-update').modal('show');
        });
