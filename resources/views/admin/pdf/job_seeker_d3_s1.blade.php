...
<body>
    <div class="row">
        ...
        <div class="col c45">
            <p class="header-title">PT. AISIN INDONESIA</p>
            ...
            <div class="c100">
                <table class="c100 description">
                    ...
                    <tr>
                        ...
                        <td>: {{ $jobSeeker->place_of_birth ?? '-' }} &
                            {{ date('d F Y', strtotime($jobSeeker->date_of_birth)) ?? '-' }}</td>
                        <td>Jenis Kelamin</td>
    ...
    <div class="row">
        <div class="col c100">
            ...
            <div class="c100">
                @if ($jobSeeker->formalEducations->count())
                    @php
                        ...
                        $sma = array_values(
                            $education->where('class', App\Models\FormalEducation::EDU_SENIOR_HIGH_SCHOOL)->toArray(),
                        )[0];
                    @endphp
                    @if ($jobSeeker->educationLevel->isDiplomaForm() || $jobSeeker->educationLevel->isBachelorForm())
                        @php
                            $d3 = array_values(
                                $education->where('class', App\Models\FormalEducation::EDU_DIPLOMA)->toArray(),
                            );
                            $d3 = isset($d3[0]) ? $d3[0] : null;
                            $s1 = array_values(
                                $education->where('class', App\Models\FormalEducation::EDU_BACHELOR)->toArray(),
                            );
                            $s1 = isset($s1[0]) ? $s1[0] : null;
                            $s2 = array_values(
                                $education->where('class', App\Models\FormalEducation::EDU_MASTER)->toArray(),
                            );
                        ...
                        <table class="c100 description bordered">
                            ...
                            <tr>
                                ...
                                <td>{{ $sma['city'] ?? '-' }}</td>
                                <td>{{ isset($sma['start_year']) && isset($sma['end_year']) ? $sma['start_year'] . ' - ' . $sma['end_year'] : '-' }}
                                </td>
                                <td>{{ $sma['grade_point'] ?? '-' }}</td>
                            </tr>
                            <tr>
                                <td>D3</td>
                                <td>{{ $d3 ? $d3['name_of_institution'] ?? '-' : '-' }}</td>
                                <td>{{ $d3 ? $d3['major']['name'] ?? '-' : '-' }}</td>
                                <td>{{ $d3 ? $d3['city'] ?? '-' : '-' }}</td>
                                <td>{{ $d3 && isset($d3['start_year']) && isset($d3['end_year']) ? $d3['start_year'] . ' - ' . $d3['end_year'] : '-' }}
                                </td>
                                <td>{{ $d3 ? $d3['grade_point'] ?? '-' : '-' }}</td>
                            </tr>
                            <tr>
                                <td>S1</td>
                                <td>{{ $s1 ? $s1['name_of_institution'] ?? '-' : '-' }}</td>
                                <td>{{ $s1 ? $s1['major']['name'] ?? '-' : '-' }}</td>
                                <td>{{ $s1 ? $s1['city'] ?? '-' : '-' }}</td>
                                <td>{{ $s1 && isset($s1['start_year']) && isset($s1['end_year']) ? $s1['start_year'] . ' - ' . $s1['end_year'] : '-' }}
                                </td>
                                <td>{{ $s1 ? $s1['grade_point'] ?? '-' : '-' }}</td>
                            </tr>
                            <tr>
                                <td>S2</td>
                                <td>{{ $s2 ? $s2['name_of_institution'] ?? '-' : '-' }}</td>
                                <td>{{ $s2 ? $s2['major']['name'] ?? '-' : '-' }}</td>
                                <td>{{ $s2 ? $s2['city'] ?? '-' : '-' }}</td>
                                <td>{{ $s2 && isset($s2['start_year']) && isset($s2['end_year']) ? $s2['start_year'] . ' - ' . $s2['end_year'] : '-' }}
                                </td>
                                <td>{{ $s2 ? $s2['grade_point'] ?? '-' : '-' }}</td>
                            </tr>
            ...
            <br>
            <p class="description">a) Uraikan dengan singkat, mengapa anda memilih jurusan tersebut di Peruruan Tinggi ?
            </p>
            <p> {{ $jobSeeker->educationDetail ? $jobSeeker->educationDetail->reason_choose_institute : '-' }} </p>
            <p class="description">b) Sebutkan Karya Ilmiah yang pernah anda buat (Skripsi, Artikel, Karya Tulis, dll)
            </p>
                    ...
                    <table class="c100 description bordered">
                        ...
                        @else
                            <tr>
                                ...
                                <td>{{ $jobSeeker->partner->place_of_birth . ', ' . $jobSeeker->partner->date_of_birth }}
                                </td>
    ...
    <div class="row">
        <div class="col c100">
            ...
            <div class="c100">
                ...
                <p class="description"><b>3. Sebutkan siapa saja yang pernah menjadi atasan anda, dan jumlah bawahan
                        saat itu</b></p>
                <div>
                    <table class="c100 description bordered">
                        <tr>
                ...
                <br>
                <p class="description">4. Masalah penting apa saja yang pernah anda hadapi, dan bagaimana mengatasinya
                    ?</p>
                ...
                <br>
                <p class="description">7. Siapakah yang mendorong anda hingga samapai pada taraf kemajuan seperti
                    sekarang ?</p>
                <p>{{ $workDetail->who_pushed ?? '-' }}</p>
                <br>
                <p class="description">8. Bagaimana bila anda menghadapi persoalan dalam pekerjaan dan harus mengambil
                    keputusan ?</p>
    ...
    <div class="row">
        ...
        <div class="c100">
            ...
            <div class="c100">
                <p class="description">1. Apakah saat ini sedang mengikuti proses rekruitment di perusahaan lain? Jika
                    Ya, Silahkan isi tabel berikut ini.</p>
                <div>
                    @php
                ...
                @endif
                <br>
                <p class="description">6. Pernahkah anda menderita penyakit yang lama sembuh (ex. TBC, Typhus,
                    Hepatitis dll) ?</p>
                <div>
                    @php
                ...
                <br>
                <p class="description">Diisi dengan sesungguhnya. Apabila dikemudian hari ternyata ada hal-hal yang
                    bertentangan, maka saya bersedia</p>
